...
global.owner = ['5565999528756'] // Put your number here
...
conn.welcome = 'Eae, @user!\n Fique No Grupo Do Dc Conosco @subject'
conn.bye = 'ESSE FDP METEU O PE =( @user!'
...
conn.onLeave = async function  ({ m, participants }) {
  ...
  for (let user of participants) {
    ...
    try {
      ...
      let text = (chat.sBye || this.bye || conn.bye || 'TCHAU @user!').replace('@user', '@' + user.split('@')[0])
      this.sendFile(m.key.remoteJid, pp, 'pp.jpg', text, m, false, {
        contextInfo: {
          mentionedJid: [user]
        }
...
Detectou @${m.participant.split`@`[0]} apagou a mensagem Para desligar este recurso, digite 
*.enable delete*
`.trim(), m.message, {
    contextInfo: {
...
if (!global.support.ffmpeg) conn.logger.warn('Instale o ffmpeg para enviar vídeos (pkg install ffmpeg)')
